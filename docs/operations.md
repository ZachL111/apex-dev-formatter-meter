...
`apex-dev-formatter-meter` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies formatter behavior through windowed input fixtures, with late-data behavior checks and no network dependency.
...
The score is deterministic. The current threshold is 156. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 3.
